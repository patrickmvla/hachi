...
export const RunTimeline = ({ steps }: RunTimelineProps) => {
  return (
    <div className="relative pl-4 border-l-2 border-border space-y-8 my-4" role="list" aria-label="Run timeline">
      {steps.map((step, index) => (
        <div key={step.id} className="relative" role="listitem">
          <div className={`
            absolute -left-[21px] top-0 w-4 h-4 rounded-full border-2 flex items-center justify-center bg-background
